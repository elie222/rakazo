...
export function AccountSettingsOverlay({
  ...
  return (
    <div className="absolute inset-0 z-30 overflow-hidden bg-overlay p-4 sm:p-10">
      <div className="flex h-full min-h-0 items-center justify-center">
        <div
          ...
          tabIndex={-1}
          className="rk-scroll max-h-full min-h-0 w-[640px] max-w-full overflow-y-auto overscroll-contain rounded-[26px] border border-border bg-card p-6 shadow-2xl sm:p-8"
        >
          <div className="flex items-start justify-between gap-6">
            <div>
              <h2 id="account-settings-title" className="text-2xl font-medium text-foreground">
          ...
          {messagingEnabled && onOpenMessaging ? (
            <section className="mt-5 rounded-[14px] border border-border bg-card px-4 py-4">
              ...
              <button
                ...
                className="mt-3 rounded-full bg-border px-4 py-2 text-[13.5px] font-medium text-foreground"
              >
              ...
              {(["robot", "organic"] as const).map((style) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 rounded-[12px] border px-3.5 py-3 text-start text-[14px] text-foreground transition-colors disabled:opacity-50 ${
                      selected ? "border-border bg-muted" : "border-border hover:border-border"
                    }`}
                  >
                    <BotAvatar
                      color="#D9508A"
          ...
          <div
            ...
            className="mt-5 rounded-[14px] border border-border bg-card px-4 py-4 outline-none"
          >
            <h3 className="text-[15px] font-medium text-foreground">
              <Trans>Usage</Trans>
            </h3>
          ...
          <details
            ...
            className="group mt-5 rounded-[14px] border border-border bg-card"
          >
  ...
  return (
    <label className="text-[12.5px] text-muted-foreground">
      ...
      <input
        ...
        className="mt-1.5 w-full rounded-[11px] border border-border bg-muted px-3.5 py-2.5 text-[14px] text-foreground outline-none focus:border-foreground/40"
      />
    </label>
  );
}
...
function AppearancePicker({
  ...
  return (
    <fieldset
      ...
      className="mt-3 grid min-w-0 grid-cols-3 gap-1 rounded-[12px] border border-border bg-muted p-1"
    >
      {options.map((option) => {
    ...
    <div ref={rootRef} className="relative mt-3">
      ...
      {open ? (
        <div
          ...
          role="listbox"
          aria-label={t`Language`}
          className="rk-scroll absolute left-0 right-0 top-full z-20 mt-2 overflow-y-auto rounded-[11px] border border-border bg-card p-1 shadow-xl"
        >
          {UI_LOCALES.map((code, index) => (
            <button
              ...
              className={`w-full rounded-[8px] px-3 py-2 text-start text-[13.5px] text-foreground outline-none hover:bg-muted focus-visible:bg-muted ${
                code === value ? "bg-muted" : ""
              }`}
