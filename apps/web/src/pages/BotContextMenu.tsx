...
export function BotContextMenu({
  ...
  return (
    <div className="fixed inset-0 z-40">
      <button
        type="button"
        aria-label={uiCopy("Close bot menu")}
      ...
      <div
        role="menu"
        aria-label={uiCopy("Actions for {name}", { values: { name: bot.name } })}
      ...
      >
        <MenuItem
          buttonRef={firstItem}
          icon={<PinIcon />}
          label={bot.pinned ? uiCopy("Unpin") : uiCopy("Pin")}
          onSelect={onTogglePinned}
        />
        <MenuItem
          icon={<FolderIcon />}
          endIcon={<ChevronIcon />}
          label={uiCopy("Move to")}
          expanded={sectionMenuOpen}
          onSelect={() => setSectionMenuOpen((open) => !open)}
        />
        <MenuItem
          icon={<ReadStatusIcon unread={bot.unread} />}
          label={bot.unread ? uiCopy("Mark as Read") : uiCopy("Mark as Unread")}
          onSelect={onToggleUnread}
        />
        <div className="my-1 border-t border-[#343438]" />
        <MenuItem icon={<EditIcon />} label={uiCopy("Edit Profile")} onSelect={onEdit} />
        <MenuItem icon={<DuplicateIcon />} label={uiCopy("Duplicate")} onSelect={onDuplicate} />
        <div className="my-1 border-t border-[#343438]" />
        <MenuItem icon={<ClearIcon />} label={uiCopy("Clear conversation")} onSelect={onClear} />
        <MenuItem icon={<ArchiveIcon />} label={uiCopy("Archive")} onSelect={onArchive} />
        <MenuItem icon={<TrashIcon />} label={uiCopy("Delete")} tone="danger" onSelect={onDelete} />
      </div>
      {sectionMenuOpen ? (
        <div
          role="menu"
          aria-label={uiCopy("Move {name} to section", { values: { name: bot.name } })}
        ...
        >
          ...
          <MenuItem
            ...
            label={uiCopy("Unassigned")}
            onSelect={() => onMoveToSection(null)}
          />
          <div className="my-1 border-t border-[#343438]" />
          <MenuItem
            icon={<NewFolderIcon />}
            label={uiCopy("New section")}
