...
Each workspace gets one Team Computer by default, so bots share its files, installed tools, and browser identity. Each Team bot starts in `bots/<bot-id>/`, while deliberately shared work belongs in `shared/`. These folders organize work but are not security boundaries: every Team bot can access the full Team workspace. A bot can instead use a Private Computer, where the whole workspace is its home. Team Computer runs use a fenced per-bot database lease, so two Team bots can work at the same time on distinct screens. Docker starts each active bot with an isolated live Chromium profile cloned from one shared profile, then checkpoints quiesced browser state back to that shared profile. Generation fencing prevents a stale concurrent bot from overwriting newer shared state, while explicitly releasing user control authoritatively saves sign-ins. Closing a viewer leaves the desktop intact; run completion or computer idle shutdown owns cleanup, so delayed window closes cannot interrupt another viewer. Sessions started or restarted after that checkpoint inherit the same Gmail and browser login state. One bot still runs only one computer-use task on its own screen. When a provider cannot spawn another display, that bot's graphical tools fail with `MULTI_SCREEN_UNAVAILABLE` instead of queueing behind a single computer lock.

Docker desktop stacks are lazy: the base display stays with the running computer, while Chromium and additional X/VNC stacks start only for bots that need them. Run completion checkpoints browser state and releases its processes. Normal computer shutdown attempts every remaining viewer checkpoint, promoting any active user controller last. A checkpoint failure is reported after stopping the computer; failed profiles remain on disk for recovery. Interrupted shared-profile swaps restore the latest saved generation before another profile is seeded or promoted. Interrupted live profiles remain on disk and are recovered when the same bot resumes; fresh profiles are cloned atomically. Screen WebSocket tokens rotate on allocation, and all view/control transports are revoked before a display is recycled. Viewer-only desktops remain available until the computer idles or stops. `SANDBOX_TEAM_SCREEN_LIMIT` can lower the active Docker screen cap from its default and maximum of eight on memory-constrained hosts.
