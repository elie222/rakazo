...
Self-hosted deployments can set `RAKAZO_SECRETS_ALLOW_PRIVATE_HTTP=1` to also allow plain `http://` origins on private network addresses (for example `http://192.168.1.20:8123`). It is off by default. Credentials sent this way are unencrypted and readable by anything on that network, so enable it only on a LAN you trust. Loopback, link-local and cloud metadata addresses stay blocked, and public origins still require HTTPS.
