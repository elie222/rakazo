...
1. The worker reconciliation loop enqueues a stable `integration.gtasks_slack.mirror` job for each user with both connectors connected, choosing the lexicographically smallest workspace where the user is still a member. Active mirror jobs are not requeued. The job rechecks workspace membership, connection status, and Composio readiness before listing tasks, then rechecks authorization before each Slack write.
