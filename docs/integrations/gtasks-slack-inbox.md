...
1. The worker reconciliation loop enqueues a stable `integration.gtasks_slack.mirror` job for each user with both connectors connected across their current member workspaces, choosing the lexicographically smallest valid workspace for execution. Active mirror jobs are not requeued. The job rechecks workspace membership, user-scoped connection status, and Composio readiness before listing tasks, then rechecks authorization before each Slack write.
