...
1. The worker reconciliation loop enqueues a stable `integration.gtasks_slack.mirror` job for each user with both connectors connected, using the lexicographically smallest workspace id when the same user is eligible in multiple workspaces. The job rechecks workspace membership, connection status, and Composio readiness before listing tasks, then rechecks authorization before each Slack write.
