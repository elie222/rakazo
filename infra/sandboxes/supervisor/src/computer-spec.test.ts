  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      try {
        ...
        expect(readlinkSync(path.join(profile, "SingletonLock"))).toBe("testhost-12345");
        if (process.platform === "linux") {
          const lock = path.join(profile, "SingletonLock");
          rmSync(lock);
          symlinkSync(`${hostname()}-${process.pid}`, lock);
          const run = () => spawnSync("bash", [path.join(root, "rakazo-browser")], {
            env: { ...process.env, DISPLAY: ":1", HOME: home,
              PATH: `${bin}${path.delimiter}${process.env.PATH ?? ""}`, RAKAZO_TEST_ARGS: capture },
            encoding: "utf8",
          });
          expect(run().status).toBe(0);
          expect(readFileSync(prefsPath, "utf8")).toContain("Crashed");
          expect(readlinkSync(lock)).toBe(`${hostname()}-${process.pid}`);
          rmSync(lock);
          ...
          expect(() => readlinkSync(lock)).toThrow();
        }
      } finally {
        rmSync(temp, { recursive: true, force: true });
      }
