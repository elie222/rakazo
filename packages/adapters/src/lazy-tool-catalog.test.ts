...
describe("lazy tool catalog", () => {
  it("exposes tools directly at the limit and switches to lazy wrappers above it", () => {
    ...
    expect(lazyCatalogTools("toolkit", "mcp", "MCP")).toHaveLength(3);
    expect(lazyCatalogTools("installed", "installed", "API").map((tool) => tool.name)).toEqual([
      "installed_search_tools",
      "installed_load_tool",
      "installed_execute_tool",
    ]);
    expect(lazyCatalogTools("toolkit", "mcp", "MCP").map((tool) => tool.name)).toEqual([
      "toolkit_search_tools",
      "toolkit_load_tool",
      "toolkit_execute_tool",
    ]);
    expect(lazyCatalogTools("toolkit", "mcp", "MCP")[0]?.description).toContain(
      "Empty query lists sources",
    );
    expect(lazyCatalogTools("toolkit", "mcp", "MCP")[0]?.inputSchema).not.toMatchObject({
  ...
  it("embeds a compact name index in the search description when it fits", () => {
    ...
    const [search] = lazyCatalogTools("toolkit", "mcp", "MCP", entries);
