...
describe("CreateOSSandboxProvider", () => {
  ...
  it("rejects screen-control changes instead of reporting a revoked lease", async () => {
    const fixture = createosFixture();
    const target = provider(fixture);

    await expect(target.setScreenControl(computer, false, context, "lease-1")).rejects.toThrow(
