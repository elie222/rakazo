...
- A message sent while one of the bot's routines was running in the same thread was folded into that routine's turn as steering, so the bot answered without the conversation ("I have no earlier context"). It now starts its own run, which waits behind the routine and answers with the full thread.
